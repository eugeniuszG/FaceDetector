...
import com.example.emotiondetector.customview.OverlayView;
import com.example.emotiondetector.env.BorderedText;
import com.example.emotiondetector.env.ImageUtils;
import com.example.emotiondetector.env.Logger;
import com.example.emotiondetector.tflite.SimilarityClassifier;
import com.example.emotiondetector.tflite.TFLiteFaceRecognition;
import com.example.emotiondetector.tracking.MultiBoxTracker;
 ...
 * objects.
 */
public class DetectorActivity extends com.example.emotiondetector.CameraActivity implements OnImageAvailableListener {
...
//  //private static final String TF_OD_API_MODEL_FILE = "facenet_hiroki.tflite";
  ...
  private void showAddFaceDialog(SimilarityClassifier.Recognition rec) {
    ...
    tvTitle.setText("Dodaj twarz");
    ivFace.setImageBitmap(rec.getCrop());
    etName.setHint("Wprowadż imię");
  ...
  private void onFacesDetected(long currTimestamp, List<Face> faces, boolean add) {
    ...
    for (Face face : faces) {
      ...
      if (boundingBox != null && goodConfidence) {
        ...
        final List<SimilarityClassifier.Recognition> resultsOutput = detector.recognizeImage(faceBmp, add);
        lastProcessingTimeMs = SystemClock.uptimeMillis() - startTime;

        if (resultsOutput.size() > 0) {

          SimilarityClassifier.Recognition result = resultsOutput.get(0);
